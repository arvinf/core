...
class TagoLightHA(TagoEntityHA, LightEntity):
    ...
    REQ_SET_BRIGHTNESS = "set_brightness"
    REQ_SET_CT = "set_ct"
    REQ_SET_HS = "set_hs"
    REQ_SET_XY = "set_xy"
    ...
    async def async_turn_on(self, **kwargs):
        ...
        if brightness is None and \
                ...
                white is None and \
                ct_colour is None:
            await self.send_request(req=self.MSG_TURN_ON)
            return
        ...
        await self.send_request(
            req='set_light', data=data
        )

    async def async_turn_off(self, **kwargs):
        await self.send_request(req=self.MSG_TURN_OFF)
...
class TagoFanHA(TagoEntityHA, FanEntity):
    FAN_ONOFF = "fan"
    FAN_DIMMABLE = "fan_dimmable"

    types = [FAN_ONOFF, FAN_DIMMABLE]
